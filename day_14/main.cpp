...
void add_value_to_memory(std::vector<std::uint64_t>& memory, int address, int value) {
    ...
    auto n_value = static_cast<std::uint64_t>(std::bitset<36>(s_value).to_ullong());
...
int main(int, char**) {
    ...
    std::cout << "Part 2: "
              << std::accumulate(
                     memory2.begin(), memory2.end(), static_cast<uint64_t>(0),
                     [](std::uint64_t acc, const std::pair<std::uint64_t, std::uint64_t>& p) {
                         return acc + p.second;
                     })
              << std::endl;
