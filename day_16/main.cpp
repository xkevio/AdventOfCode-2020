...
bool is_in_range(int num, const Range& r) {
    return (num >= r.first_min && num <= r.first_max) ||
           (num >= r.second_min && num <= r.second_max);
}
...
void remove_multiple_ranges(std::unordered_map<int, std::vector<std::string>>& order) {
    ...
    while (treated.size() < 20) {
        for (auto& p : order) {
            if (p.second.size() == 1 &&
                std::find(treated.begin(), treated.end(), p.first) == treated.end()) {
                word = p.second[0];
...
std::uint64_t get_departures(const std::vector<std::string>& valid, const std::unordered_map<std::string, Range>& ranges_, const std::string yt) {
